...
def can_partition_td(arr):
    ...
    if S%2:
        return False
    dp = [[-1 for _ in range(S//2+1)] for _ in range(len(arr))]
...
def can_partition_td_recursive(dp, arr, desired_sum, currentIndex):
    ...
    if not (dp[currentIndex][desired_sum]+1):
        ...
        dp[currentIndex][desired_sum] = can_partition_recursive(arr, desired_sum, currentIndex+1)
