...
def min_deletions_recursive(nums, currentIdx, previousIdx):
    ...
    if previousIdx == -1 or nums[currentIdx] > nums[previousIdx]:
        with_current = 1 + min_deletions_recursive(nums, currentIdx+1, currentIdx)
    without_current = min_deletions_recursive(nums, currentIdx+1, previousIdx)
...
def min_deletions_td_recursive(dp, nums, currentIdx, previousIdx):
    ...
    if dp[currentIdx][previousIdx] == -1:
        with_current = 0
        if previousIdx == -1 or nums[currentIdx] > nums[previousIdx]:
            with_current = 1 + min_deletions_td_recursive(dp, nums, currentIdx+1, currentIdx)
        without_current = min_deletions_td_recursive(dp, nums, currentIdx+1, previousIdx)
        dp[currentIdx][previousIdx] = max(with_current, without_current)
    return dp[currentIdx][previousIdx]

# O(n^2s) time | O(n) space
