...
# O(3^(m+n)) time | O(m+n) space; m=len(s1), n=len(s2)
...
def find_LCS_length_recursive(s1, s2, i, j, count):
    ...
    return max(count, withoutI, withoutJ)
