...
`Problem: Delete Duplicate Rows (based on a certain keys from table)
Task: Identify and Delete rows from below SQL table with duplicates, keep only the latest 
Employees
+------+------------+---------+---------- -+
| Name | Department | Country | InsertDate |
+------+------------+---------+---------- -+
Keys: Name, Department, Country 
Latest On: InsertDate
`
WITH ranked_rows AS (
  SELECT 
    RANK() OVER(PARTITION BY Name, Department, Country ORDER BY InsertDate DESC) ranking
  FROM 
    Employees
)
DELETE 
FROM ranked_rows
WHERE ranking > 1; -- ranking <> 1
